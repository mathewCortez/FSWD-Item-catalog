...
# JSON APIs to view Hobby Information
@app.route('/hobby/<int:hobby_id>/menu/JSON')
...
def hobbyItemJSON(hobby_id, menu_id):
    ...
    return jsonify(Hobby_Item=Hobby_Item.serialize)
...
def deleteHobby(hobby_id):
    hobbyToDelete = session.query(Hobby).filter_by(id=hobby_id).one()
    if 'username' not in login_session:
        return redirect('/login')
...
def showMenu(hobby_id):
    ...
    items = session.query(HobbyItem).filter_by(hobby_id=hobby_id).all()
...
def newHobbyItem(hobby_id):
    ...
    if request.method == 'POST':
        newItem = HobbyItem(name=request.form['name'], description=request.form['description'], price=request.form[
                           'price'], hobby_id=hobby_id, user_id=hobby.user_id)
        session.add(newItem)
        session.commit()
        flash('New Hobby %s Item Successfully Created' % (newItem.name))
